...
class Hypervisor extends Sim_entity {
 ...
 //   add_param(new Anim_param("Count", Anim_param.NAME_VALUE, "0", -10, -5));
  ...
  public void body() {
    ...
    count = count-1;
	coef = 0.75;
	cpuVM = Double.parseDouble(hc.getHostCPUSpeed()) * (Double.parseDouble(hc.getHostCPUCores())* coresPersentage) * coef / number_of_vms;
	if (cpuVM<=500)
	{
		System.out.println("Resources are too low in " + get_name() + " for creating VMs");
	}else{
	System.out.println(f+" "+get_name() +": "+ count +" events,  VMs created: "+number_of_vms +", cores per VM: " + (Double.parseDouble(hc.getHostCPUCores())* coresPersentage) + ", CPU speed per VM: "+ cpuVM);
	}
  }
}
